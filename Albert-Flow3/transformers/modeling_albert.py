...
class ALBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # for vq
        self.codebook = VQEmbedding(64,768)
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None,output_all_encoded_layers=True):
        ...
        for i in range(self.layer_num):
            ...
            z_q_x_st, z_q_x = self.codebook.straight_through(hidden_states)
            z_q_x = z_q_x.squeeze(0).permute(1,2,0).contiguous()
            vq_loss += F.mse_loss(z_q_x, hidden_states.detach())
            vq_loss += F.mse_loss(hidden_states, z_q_x.detach())
            hidden_states = z_q_x_st.permute(1,2,0).contiguous()
        ...
        return outputs, vq_loss  # last-layer hidden state, (all hidden states), (all attentions)
