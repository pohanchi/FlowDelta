...
class ALBertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states_0,flow=None):
        hidden_states = self.dense(hidden_states_0)
        hidden_states = self.intermediate_act_fn(hidden_states)
        layer_output = self.output(hidden_states, hidden_states_0,flow)
...
class ALBertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor,flow=None):
        ...
        if flow is not None:
            hidden_states = hidden_states + flow
        return hidden_states
...
class ALBertLayer(nn.Module):
    ...
    def forward(self,idx, hidden_states, attention_mask=None, head_mask=None,flow=None):
        if (idx == 0):
        ...
        layer_output = self.intermediate(attention_norm,flow=flow)
        ...
        for i in range(self.layer_num):
            ...
            if self.use_flow:
                if i == self.layer_num-1:
                    flow_out = self.flow_op(hidden_states, self.flow)

            if self.output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)

            layer_outputs = self.transformer(i,hidden_states, attention_mask, head_mask[i],flow=flow_out)
